...
def get_server_settings(base_url="http://localhost:4000"):
    ...
    try:
        req = urllib.request.Request(
            f"{base_url}/settings", headers={"User-Agent": "Claude-Code-Hook/1.0"}
        )
...
def main():
    ...
    parser.add_argument(
        "--source-app",
        help="Source application name (optional, auto-detected from config if not provided)",
    )
    parser.add_argument(
        "--event-type",
        required=True,
    ...
    if not source_app:
        ...
        if config and "PROJECT_NAME" in config:
            ...
            print(
                f"Warning: PROJECT_NAME not in config, using directory name: {source_app}",
                file=sys.stderr,
            )
