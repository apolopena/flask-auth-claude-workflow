...
class HITLRequest:
    ...
    def _start_response_server(self):
        ...
        try:
            ...
            print(
                "Warning: websockets package not installed. Install with: pip install websockets"
            )
            return
    ...
    def send_and_wait(
        self, hook_event_data: Dict[str, Any], session_data: Dict[str, Any]
        ...
        try:
            ...
            response = requests.post(
                f"{self.observability_url}/events", json=event_payload, timeout=10
            )
            response.raise_for_status()
        except ImportError:
            print(
                "Warning: requests package not installed. Install with: pip install requests"
            )
            return None
        except Exception as e:
...
def ask_question(
    question: str, session_data: Dict[str, Any], timeout: int = 300
) -> Optional[str]:
    ...
    response = hitl.send_and_wait(
        {"hook_event_type": "HumanInTheLoop", "payload": {}}, session_data
    )
...
def ask_choice(
    question: str, choices: list[str], session_data: Dict[str, Any], timeout: int = 300
) -> Optional[str]:
    """Ask user to choose from options"""
    hitl = HITLRequest(question, hitl_type="choice", choices=choices, timeout=timeout)
    response = hitl.send_and_wait(
        {"hook_event_type": "HumanInTheLoop", "payload": {}}, session_data
    )
